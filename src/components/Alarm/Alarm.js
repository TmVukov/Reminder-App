  ...
  return (
    <div className="alarm__container">
      ...
      <div className="alarm__message">
        {!alarmDate && !alarmTime && discarded ? (
          <p>Alarm discarder. Please add new alarm.</p>
        ) : !alarmDate && !alarmTime ? (
          ''
        ) : !alarmDate ? (
          <p>Please select date.</p>
        ) : reminders.some((e) => e.alarmDisplay[0] === alarmDisplay[0]) ? (
          <p>Alarm is already set!</p>
        ) : alarmDisplay.length &&
          convertDateToMs(alarmDisplay) < new Date().getTime() ? (
          <p>Alarm is not valid! Please discard it.</p>
        ) : alarmDisplay.length ? (
          <p>Alarm is saved!</p>
        ) : alarmDisplay && alarmTime ? (
          <p>
            Save alarm at {alarmDate}/{alarmTime.slice(0, 5)}?
          </p>
        ) : (
          ''
        )}
        ...
        {alarmDate && alarmTime && (
          <div>
            <button
              className="alarm__icon-check"
              disabled={alarmDisplay.length ? true : false}
            >
              <AiOutlineCheck onClick={startAlarm} />
            </button>

            <AiOutlineClose
              onClick={discardAlarm}
              className="alarm__icon-close"
            />
          </div>
        )}
      </div>
